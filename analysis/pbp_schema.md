...
| `distance` | integer | Yards to go, parsed from `situation`. | NA wherever `down` is NA. On a `goal_to_go` situation, `distance` is set to `yard_num` (see below), since "and Goal" only ever occurs deep enough in the opponent's territory that the yardline number itself is the distance to the goal. |
| `goal_to_go` | logical | TRUE on "and Goal" situations (e.g. `"1st and Goal at CMU06"`), parsed from `situation`. | NA wherever `down` is NA (no situation to read); FALSE for an ordinary numeric down-and-distance. |
